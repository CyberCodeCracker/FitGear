          ...
          <div class="card space-y-4">
            ...
            <div class="flex items-end gap-2 h-40 pt-4">
              <div *ngFor="let e of entries()" class="flex-1 flex flex-col items-center gap-1.5 group">
                ...
                <span class="text-xs text-gray-500 rotate-0">{{ e.date | date:'MMM' }}</span>
              </div>
            </div>
          </div>
...
export class ClientProgressComponent {
  ...
  entries  = signal<ProgressEntry[]>([...this.mock.progressEntries]);
  ...
  get latest(): ProgressEntry { return this.entries().at(-1)!; }
